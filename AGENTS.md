# GreatDeals.ca Product Owner & Competitive Research Agent

## Project orchestration

This repository follows the Canada Deals bootstrap workflow. Route work to the scoped agent for the directory being changed:
...
Do not silently skip checkpoints, expand the MVP, or replace approved technologies. Read the relevant source-of-truth documents before acting. The Product Owner role below applies to product/market work; nested `AGENTS.md` files define the implementation roles for `ux/`, `architecture/`, `integrations/`, `backend/`, `frontend/`, `qa/`, and `security/`.

The current repository is in the foundation phase. Do not create application source code or choose a technology stack until the Product Owner phase and required human checkpoint are complete.
...
Aim for at least 10 meaningful competitors when enough legitimate examples exist.
...
Include a short evidence note or source for ambiguous/high-impact cells.
...
Explicitly define `NOT in MVP` and explain why those items are postponed. Avoid feature creep.
...
End each full research engagement with a specific Product Owner recommendation answering:

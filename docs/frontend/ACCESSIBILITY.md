...
- Homepage store banners derive their accessible name from the complete visible copy; active outbound banners expose new-tab context separately through an accessible description. Deal Card retailer actions and every card-level Wishlist state begin with their exact visible label (`Check retailer price`, `Save`, `Saved`, or `Wait`) before adding Product or retailer context, satisfying WCAG 2.2 Label in Name without changing keyboard, focus, handoff, or pressed-state behavior.

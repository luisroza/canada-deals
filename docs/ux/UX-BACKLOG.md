...
| UX-003 | Deals feed | Scan credible deals quickly | P0 | Feed shows current price, retailer, freshness, evidence state, and match confidence on every standard card; ranking does not hide unknown states. |
...
| UX-006 | Product Page | Verify the exact product before purchase | P0 | Page identifies title/variant, current CAD price, retailer, observation time, evidence, confidence, and next action above the fold. |
| UX-007 | Price history | Understand whether price is meaningful | P0 | Reliable, partial, and unavailable states are distinct; unavailable history has no fake chart; chart has text alternative. |
...
| UX-009 | Retailer handoff | Continue purchase with clear expectations | P0 | CTA identifies retailer, external navigation, current price context, and nearby affiliate disclosure; no paid-ranking implication. |
...
| UX-021 | UX research | Validate Canadian shopping comprehension | P0 | Research tests trust, freshness, safe comparison, history states, disclosure, and alert expectations with target users before UX approval. |
...
The UX phase is ready for Human UX Checkpoint when the four UX documents, research evidence, responsive wireframes, state coverage, accessibility requirements, and open decisions are reviewed together. Approval does not authorize architecture or implementation; those are the next separately governed phases.

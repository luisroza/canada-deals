...
The editor follows three visible steps: **Analyze link**, **Review card**, and **Publish**. It starts in a safe draft/manual-review state and never publishes from link inspection alone. The owner can then create a new Product or add an independent offer to an existing Product identity. Reuse keeps catalog normalization stable without merging, ranking, or comparing the retailer offers. Product slugs are immutable after creation so legacy saved and shared routes remain stable.

Frequently used Product and offer essentials are open by default, while source/retailer metadata remains in one collapsed advanced section. The manual editor intentionally omits MPN, GTIN, variant JSON, condition, pack quantity, and bundle contents: those normalization fields are retained in backend contracts for existing data and approved connectors but are not required to publish an independent offer. New manual offers keep safe neutral defaults, and editing an existing offer preserves its stored values. The visible contract focuses on title/slug/brand/category/model, retailer/policy/external ID/SKU/original title/Product URL/seller/marketplace/external identifiers, deal and regular CAD prices, evidence and observation times, optional promotion validity, availability/region/shipping, review state, and publication state. Brand begins unselected for a new Product so the first active database record is never silently assigned. Link validation selects a matching active Brand when available. Otherwise it may fill a clearly labelled, editable low-confidence candidate without creating a catalog record. The owner must confirm that candidate; creation or reactivation then occurs with the offer save. The optional validity time automatically removes an expired offer from public discovery and retailer handoff without requiring a scheduled job.
...
**Catalog** is a focused Category-management screen because Categories control public discovery structure. Stores retain their independent top-level operational area. Both use responsive cards rather than the Offers table, with name/key search and All, Active, Inactive, With public offers, and Empty filters. Counts are projected by the backend from the complete database rather than inferred from the dashboard's bounded offer list. There is no standalone Brands tab: Brand detection, matching, and confirmed creation/reactivation stay inside Offer entry.
...
All new categories and stores begin inactive. The explicit Offer-save confirmation defined by ADR-012 may create or reactivate a reviewed Brand as active in the same transaction as the offer; it does not expose a separate Brand-management journey. Category and Store display names may be edited, while category slugs and store keys remain immutable after creation. There is no destructive delete action. Deactivating a category hides its products and offers from public discovery while preserving products, offers, wishlists, history, and audit. Deactivating a store hides its offers and banners and blocks product/store handoffs while preserving listings, programs, links, destinations, artwork, history, and audit. Existing Brand identity remains governed by domain and connector rules.
...
- Categories and Stores: active, inactive, empty/no public offers, saving, conflict, and impact warning. Inline Brand intake adds matched, candidate, confirmation-required, conflict, and save-failed states.

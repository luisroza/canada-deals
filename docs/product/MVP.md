...
- Owner administration manages Categories and Stores as standalone catalog screens. Brand detection, matching, and explicitly confirmed creation/reactivation occur only inside Offer entry; the canonical Brand entity, normalized uniqueness, existing Product identity, and connector ingestion remain intact. Category slugs and store keys are immutable; deactivation is audited and reversible; no linked Product, offer, Wishlist, banner, affiliate, or history data is deleted. Inactive brands, categories, and stores fail closed across public discovery and handoff.

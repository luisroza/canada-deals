...
Application Implementation — Owner Catalog Workflow Refinement — IMPLEMENTED AND VALIDATED; OWNER BOOTSTRAP AND LIVE MERCHANT ACTIVATION BLOCKED
...
- Owner Brand management, canonical Product reuse for additional store offers, immutable Product slugs, optional automatic offer expiry, progressive offer entry, and single-source carousel selection
...
- Persistence: PostgreSQL/Npgsql through `20260824212134_AddAdminCatalogWorkflow`; local development uses Docker Compose.
...
- Owner administration: separate responsive shell for Overview, Offers, Brands, Categories, Stores, Store Banners, customer Reports, and Audit. Brand/Category/Store operations create inactive records, preserve immutable slug/key identity, expose database-wide impact counts, require reasons for deactivation, and never delete linked records. Offer entry can create a Product or reuse a canonical Product for an additional store offer; Product slugs remain immutable, essentials and advanced metadata are separated, and an optional validity cutoff removes expired offers from public reads and handoff without a job. Banner membership is controlled only by the audited selection workflow; the banner editor manages content, order, artwork, and rights. All APIs require the `OwnerAdmin` role; writes require CSRF, rate limiting, bounded validation, and transactional audit. Bootstrap is interactive and no credential is committed.
- Tests: the current revision passes 87 domain tests, 155 isolated PostgreSQL integrations, 86 frontend tests, and 26 full-stack Playwright journeys with zero skips; the backend build has zero warnings/errors and the Next.js production build passes. The complete migration chain applied to a new dedicated PostgreSQL database during integration startup, and a second migration-only run reported the database current. The new coverage proves Brand lifecycle, canonical Product reuse without duplication, immutable slugs, automatic expired-offer exclusion, progressive admin offer entry, and single-source carousel selection. Product-image coverage continues to prove pending/active/archive publication, public projection, binary delivery, rights/date/placement domain gates, and UI image rendering. PostgreSQL integrations run serially because they intentionally share one isolated database fixture.

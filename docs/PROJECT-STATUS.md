...
- Persistence: PostgreSQL/Npgsql through `20260824152343_AddAdminCategoryAndRetailerManagement`; local development uses Docker Compose.
...
- Tests: the current revision passes 82 domain tests, 69 frontend tests, 7 focused PostgreSQL owner-panel integrations including Category/Store management, the API/integration build with zero warnings, and the Next.js production build. Earlier full-suite evidence remains recorded below; the historical parallel-only `recent` sort concurrency observation is unchanged.

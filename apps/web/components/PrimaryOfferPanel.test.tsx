...
describe("PrimaryOfferPanel", () => {
  it("puts the current price, freshness, retailer action, and disclosure in one decision block", () => {
    ...
    expect(screen.getByRole("heading", { name: "Offer conditions" })).toBeInTheDocument();
    expect(screen.getByText("Shipping calculated at checkout")).toBeInTheDocument();
    expect(screen.getByText(/No verified requirement was supplied/)).toBeInTheDocument();
    expect(screen.getByText(/No verified expiry was supplied/)).toBeInTheDocument();
  });
});
